...
describe('test api integration', () => {
  ...
  it('test POST /login returns for valid response', () => new Promise((done) => {
    request.post(`${url}/login`, { json: { userName: 'Popsicool' } }, (_err, res, body) => {
      expect(res.statusCode).to.be.equal(200);
      expect(body).to.be.equal('Welcome Popsicool');
      done();
    });
  }));
